...
<div class="facturaDatos">
    <form action="{{route('facturas.update',$factura->numero)}}" method="post">
        ...
        @method('put')
            ...
            Iva <input type="number" name="iva" step="0.01" value="{{$factura->iva}}" disabled>
...
<div class="facturaProducto">
    <form action="{{route('lineas.store')}}" method="post" id="form_linea">
        ...
        <button type='submit' class="btn btn-warning" name="Añadir">Añadir</button>
        <br>
    </form>
</div>
...
<div class="facturaTabla">
    ...
    <tr><td>{{$linea->descripcion}}</td>
        ...
        <?php $sumaTotal += $linea->cantidad*$linea->precio; ?>
    </tr>
    
    @endforeach
    <tr>
        <td colspan="5">Suma <strong>Total</strong>: {{$sumaTotal+(($factura->iva*$sumaTotal)/100)}}</td>
    </tr>
